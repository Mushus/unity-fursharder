...
/**
 * 画面上の点 viewDir のオブジェクト座標系の法線 normal をビュー座標のノーマルマップに補正する
 * カメラ中央で捉えたときの法線が返る
 */
float3 calcViewNormal(float3 normal, float3 viewDir) : float3 {
    ...
    // NOTE: normalize されるので回転行列を簡略化できる
...
float4 frag(vert2frag i) : COLOR {
    ...
    // HACK: Unityのドキュメントからコピペしてきただけのイケてないコード
    // Diffとはなんぞや、なぜ加算しているのかを説明的にリファクタする
